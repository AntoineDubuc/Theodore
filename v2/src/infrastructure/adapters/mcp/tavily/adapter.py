...
Implements the MCP search tool port interface using Tavily AI's advanced
...
class TavilyAdapter(
    ...
    """
    Production-ready Tavily AI MCP search adapter.
    ...
    def _build_company_search_query(
        ...
        if company_description:
            query_parts.append(f"business: {company_description[:150]}")
        ...
        if filters:
            ...
            if filters.funding_stage:
                filter_parts.append(f"funding {filters.funding_stage}")
            
            if filters.technologies:
                tech_list = " ".join(filters.technologies[:3])  # Limit to top 3
                filter_parts.append(f"technology {tech_list}")
            
            if filters.keywords:
                keyword_list = " ".join(filters.keywords[:3])  # Limit to top 3
                filter_parts.append(keyword_list)
        ...
        query_parts.append(" ".join(enhancement_keywords[:3]))
    ...
    def _build_keyword_search_query(
        ...
        company_terms = ["company", "business", "startup", "enterprise"]
        query_parts.append(" ".join(company_terms[:2]))
    ...
    def _extract_companies_from_response(
        ...
        """
        Extract company information from Tavily response using AI parsing.
        
        Args:
            response_content: Response content from Tavily
            results: Search results with structured data
        ...
        for result in results:
            ...
            for company_name in company_names:
                # Extract website
                website = self._extract_website(url, content, company_name)
                ...
                company = Company(
                    name=company_name,
                    website=website or f"https://example.com/{company_name.lower().replace(' ', '-')}",
        ...
        # Process answer content if available
        if response_content:
            answer_companies = self._extract_companies_from_text(response_content)
    ...
    def _extract_company_names(self, title: str, content: str) -> List[str]:
        ...
        patterns = [
            # Company with suffixes (case insensitive)
            r'(\b[A-Z][a-zA-Z0-9&\s]+?)\s+(?:Inc\.?|LLC\.?|Corp\.?|Corporation|Ltd\.?|Company|Co\.?)\b',
            # Company name followed by "is/was a company/business/startup"
            r'(\b[A-Z][a-zA-Z0-9&\s]+?)\s+(?:is|was)\s+a\s+(?:leading\s+)?(?:company|business|startup|corporation)',
            # Company name in title format
            r'^([A-Z][a-zA-Z0-9&\s]+?)\s+-\s+',
            # Founded pattern
            r'founded\s+([A-Z][a-zA-Z0-9&\s]+?)\s+in',
            # Company provides/offers pattern
            r'(\b[A-Z][a-zA-Z0-9&\s]{2,20}?)\s+(?:provides|offers|specializes)',
        ]
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if len(name) < 2 or name.lower() in [
                    'the', 'this', 'that', 'company', 'business', 'enterprise', 'a', 'an',
                    'test', 'example', 'sample'
                ]:
                    continue
                
                # Skip very long names (likely extraction errors)
                if len(name) > 50:
                    continue
    ...
    def _extract_companies_from_text(self, text: str) -> List[Company]:
        """Extract companies from text content."""
        companies = []
        ...
        for sentence in sentences:
            company_names = self._extract_company_names(sentence, "")
    ...
    def _extract_website(self, url: str, content: str, company_name: str) -> Optional[str]:
        """Extract website from URL or content."""
        if url and url.startswith('http'):
            return url
        
        # Look for website mentions in content
        website_pattern = r'(?:https?://)?(?:www\.)?([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})'
        matches = re.findall(website_pattern, content.lower())
        
        for match in matches:
            # Check if domain is related to company name
            if any(part.lower() in match for part in company_name.lower().split()):
                return f"https://{match}"
        
        return None
    
    def _extract_company_description(self, content: str, company_name: str) -> Optional[str]:
    ...
    def _calculate_extraction_confidence(
        ...
        if website and not website.startswith("https://example.com"):
            confidence += 0.2
        
        # Boost from search relevance score
        confidence += search_score * 0.2
    ...
    async def search_similar_companies(
        ...
        try:
            ...
            companies = self._extract_companies_from_response(
                response.answer or "", response.results, query
            )
            ...
            if companies:
                ...
                for i, company in enumerate(companies):
                    ...
                    if i < len(response.results):
                        search_score = response.results[i].get("score", 0.0)
            ...
            result = MCPSearchResult(
                ...
                metadata={
                    ...
                    "tavily_metadata": response.search_metadata
                },
                citations=[result.get("url", "") for result in response.results],
            ...
            raise MCPRateLimitedException("tavily", e.retry_after)
        except TavilyQuotaError:
            raise MCPQuotaExceededException("tavily", "requests")
        except TavilyAuthError as e:
            raise MCPConfigurationException("tavily", str(e))
        except asyncio.TimeoutError:
            raise MCPSearchTimeoutException("tavily", self.config.timeout_seconds)
        except Exception as e:
            raise MCPSearchException(f"Tavily search failed: {str(e)}")
    ...
    async def search_by_keywords(
        ...
        try:
            ...
            raise MCPSearchException(f"Tavily keyword search failed: {str(e)}")
    ...
    async def search_similar_companies_streaming(
        ...
        # For Tavily, we get all results at once, but can yield them progressively
